...
class EnvParamsAll(EnvParams):
    puzzle: PushWorldPuzzleAll = struct.field(pytree_node=True, default=None)


class MetaTaskPushWorldEnvironmentAll(Environment[EnvParamsAll, TimeStepAll[EnvCarry], StateAll[EnvCarry]]):
    def default_params(self, **kwargs: Any) -> EnvParamsAll:
        params = EnvParamsAll()
        params = params.replace(**kwargs)
        return params

    # We assume level 0 only for now so we limit the shape
    def observation_shape(self, params: EnvParamsAll) -> tuple[int, int, int] | dict[str, Any]:
        return (
            LEVEL0_ALL_SIZE,
            LEVEL0_ALL_SIZE,
            LEVEL0_NUM_CHANNELS,
    ...
    def reset(self, params: EnvParamsAll, key: jax.Array) -> TimeStepAll[EnvCarry]:
    ...
    def eval_reset(self, params: EnvParamsAll, key: jax.Array) -> TimeStepAll[EnvCarry]:
        return self.reset(params, key)

    def step(self, params: EnvParamsAll, timestep: TimeStepAll[EnvCarry], action: IntOrArray) -> TimeStepAll[EnvCarry]:
    ...
    def _generate_problem(self, params: EnvParamsAll, key: jax.Array) -> StateAll[EnvCarry]:
        state = StateAll(
            key=key,
